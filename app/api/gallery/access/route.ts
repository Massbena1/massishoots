...
// GET — galeries publiques
export async function GET() {
  const galleries = loadGalleries();
  const pub = galleries.filter(g => g.actif && g.public).map(({ code: _code, ...rest }) => rest);
  return NextResponse.json(pub);
}

// POST — vérifier un code privé
...
// PUT — notifier quand galerie introuvable
