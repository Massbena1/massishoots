...
export default function AdminClientsPage() {
  ...
  const addDeliverable = (target: ClientPortal | typeof form, setTarget: (v: any) => void) => {
    const d: Deliverable = { id: Date.now().toString(), label: "", status: "pending", deliveredDate: "", downloadUrl: "", note: "" };
    setTarget({ ...target, deliverables: [...(target.deliverables ?? []), d] });
  };

  const updateDeliverable = (target: ClientPortal | typeof form, setTarget: (v: any) => void, idx: number, key: keyof Deliverable, val: string) => {
    const deliverables = [...(target.deliverables ?? [])];
    deliverables[idx] = { ...deliverables[idx], [key]: val };
    setTarget({ ...target, deliverables });
  };

  const removeDeliverable = (target: ClientPortal | typeof form, setTarget: (v: any) => void, idx: number) => {
    setTarget({ ...target, deliverables: (target.deliverables ?? []).filter((_, i) => i !== idx) });
  };

  const addInvoice = (target: ClientPortal | typeof form, setTarget: (v: any) => void) => {
    const inv: Invoice = { id: Date.now().toString(), label: "", amount: 0, status: "en_attente", date: "", url: "" };
    setTarget({ ...target, invoices: [...(target.invoices ?? []), inv] });
  };

  const updateInvoice = (target: ClientPortal | typeof form, setTarget: (v: any) => void, idx: number, key: keyof Invoice, val: string | number) => {
    const invoices = [...(target.invoices ?? [])];
    invoices[idx] = { ...invoices[idx], [key]: val };
    setTarget({ ...target, invoices });
  };

  const removeInvoice = (target: ClientPortal | typeof form, setTarget: (v: any) => void, idx: number) => {
    setTarget({ ...target, invoices: (target.invoices ?? []).filter((_, i) => i !== idx) });
  };

  const ClientForm = ({ data, setData, onSubmit, submitLabel }: { data: any; setData: (v: any) => void; onSubmit: (e: React.FormEvent) => void; submitLabel: string }) => (
    <form onSubmit={onSubmit}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 8 }}>
        {[["name", "Nom *"], ["company", "Entreprise"], ["email", "Email *"], ["phone", "Téléphone"], ["password", "Code d'accès *"]].map(([k, l]) => (
          <input key={k} required={l.includes("*")} placeholder={l} value={data[k] ?? ""} onChange={e => setData({ ...data, [k]: e.target.value })} style={inputStyle} />
        ))}
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 8 }}>
        ...
        <input placeholder="Prochaine étape (ex: Tournage Mois 2)" value={data.nextStep ?? ""} onChange={e => setData({ ...data, nextStep: e.target.value })} style={inputStyle} />
      ...
      <textarea placeholder="Livrables inclus (une ligne par livrable)" rows={3} value={(data.serviceIncludes ?? []).join("\n")} onChange={e => setData({ ...data, serviceIncludes: e.target.value.split("\n").filter(Boolean) })} style={{ ...inputStyle, resize: "vertical", marginBottom: 8 }} />
      ...
      {(data.deliverables ?? []).map((d: Deliverable, i: number) => (
        <div key={d.id} style={{ display: "grid", gridTemplateColumns: "1.5fr 2fr 1fr 1fr 2fr auto", gap: 6, marginBottom: 6, alignItems: "center" }}>
          ...
          <button type="button" onClick={() => removeDeliverable(data, setData, i)} style={{ background: "rgba(248,113,113,0.1)", border: "1px solid rgba(248,113,113,0.2)", borderRadius: 6, padding: "6px 8px", cursor: "pointer", color: "#f87171", flexShrink: 0 }}>
            <X size={12} />
          </button>
        </div>
      ))}
      <button type="button" onClick={() => addDeliverable(data, setData)} className="font-dm" style={{ fontSize: 11, color: "rgba(255,255,255,0.35)", background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 7, padding: "5px 12px", cursor: "pointer", marginBottom: 4 }}>
        + Ajouter un livrable
      </button>
      ...
      {(data.invoices ?? []).map((inv: Invoice, i: number) => (
        <div key={inv.id} style={{ display: "grid", gridTemplateColumns: "2fr 1fr 1fr 1fr 2fr auto", gap: 6, marginBottom: 6, alignItems: "center" }}>
          ...
          <button type="button" onClick={() => removeInvoice(data, setData, i)} style={{ background: "rgba(248,113,113,0.1)", border: "1px solid rgba(248,113,113,0.2)", borderRadius: 6, padding: "6px 8px", cursor: "pointer", color: "#f87171", flexShrink: 0 }}>
            <X size={12} />
          </button>
        </div>
      ))}
      <button type="button" onClick={() => addInvoice(data, setData)} className="font-dm" style={{ fontSize: 11, color: "rgba(255,255,255,0.35)", background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 7, padding: "5px 12px", cursor: "pointer", marginBottom: 4 }}>
        + Ajouter une facture
      </button>
      ...
      <textarea placeholder="Notes internes (non visible par le client)" rows={2} value={data.internalNote ?? ""} onChange={e => setData({ ...data, internalNote: e.target.value })} style={{ ...inputStyle, resize: "vertical", marginBottom: 8 }} />
      {data.clientNote && (
      ...
      <div style={{ display: "flex", gap: 8, marginTop: 12 }}>
        <button type="submit" disabled={loading} className="font-dm" style={{ display: "flex", alignItems: "center", gap: 5, padding: "10px 20px", background: "#4ade80", color: "#0a0a0a", border: "none", borderRadius: 8, fontSize: 12, fontWeight: 700, cursor: "pointer" }}>
          <Check size={12} /> {loading ? "..." : submitLabel}
        </button>
        <button type="button" onClick={() => { setShowAdd(false); setEditClient(null); }} className="font-dm" style={{ padding: "10px 16px", background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 8, color: "rgba(255,255,255,0.4)", fontSize: 12, cursor: "pointer" }}>
          Annuler
        </button>
      </div>
    </form>
      ...
      <div style={{ maxWidth: 1000, margin: "0 auto" }}>
        ...
        {showAdd && (
          <div style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 20, padding: "24px", marginBottom: 24 }}>
            ...
            <ClientForm data={form} setData={setForm} onSubmit={handleAdd} submitLabel="Créer l'espace" />
          </div>
        )}
            ...
            <div key={c.id} style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 14, overflow: "hidden" }}>
              {editClient?.id === c.id ? (
                <div style={{ padding: "20px" }}>
                  ...
                  <ClientForm data={editClient} setData={setEditClient} onSubmit={(e) => { e.preventDefault(); handleSave(); }} submitLabel="Enregistrer" />
                </div>
              ) : (
                <div>
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "14px 18px", cursor: "pointer" }} onClick={() => setExpanded(expanded === c.id ? null : c.id)}>
                    <div>
                      ...
                      <p className="font-dm" style={{ fontSize: 11, color: "rgba(255,255,255,0.3)", marginTop: 2 }}>
                        Code: {c.password} · {PHASES.find(p => p.id === c.currentPhase)?.label} · {c.deliverables?.length ?? 0} livrables · {c.invoices?.length ?? 0} factures
                      </p>
                    </div>
                    <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                      <button onClick={e => { e.stopPropagation(); setEditClient({ ...c }); setShowAdd(false); }} style={{ background: "rgba(196,205,214,0.08)", border: "1px solid rgba(196,205,214,0.15)", borderRadius: 8, padding: "5px 7px", cursor: "pointer", color: "#c4cdd6", display: "flex" }}>
                        <Pencil size={13} />
                      </button>
                      <button onClick={e => { e.stopPropagation(); handleDelete(c.id); }} style={{ background: "rgba(248,113,113,0.1)", border: "1px solid rgba(248,113,113,0.2)", borderRadius: 8, padding: "5px 7px", cursor: "pointer", color: "#f87171", display: "flex" }}>
                        <Trash2 size={13} />
                      </button>
                        ...
                        <div>
                          ...
                          {(c.deliverables ?? []).map(d => (
                            <p key={d.id} className="font-dm" style={{ fontSize: 11, color: "rgba(255,255,255,0.45)", marginBottom: 3 }}>
                              {d.label} — <span style={{ color: d.status === "delivered" ? "#4ade80" : d.status === "in_progress" ? "#f59e0b" : "rgba(255,255,255,0.3)" }}>{d.status === "delivered" ? "Livré" : d.status === "in_progress" ? "En cours" : "À venir"}</span>
                            </p>
                          ))}
                        </div>
                        <div>
